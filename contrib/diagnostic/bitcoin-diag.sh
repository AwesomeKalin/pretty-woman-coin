...
get_grep_prettywomancoind()
{
    ps_out=$(ps aux)
    ps_err=$?
    if [[ ps_err -eq 0 ]]; then
        echo "$ps_out" | ( grep prettywomancoind; if [[ $? -eq 0 || $? -eq 1 ]]; then exit 0; else exit 1; fi ) | awk '
        BEGIN { ORS = ""; print " ["}
...
#Default values
BITCOINCLI="prettywomancoin-cli"
...
if [[ $HELP == YES ]]; then
    echo "Prettywomancoin diagnostic help"
    echo ""
    echo "Parameters: "
    echo "      --rpcuser       (default=\"\")             RPC user"
    echo "      --rpcpassword   (default=\"\")             RPC password"
    echo "      --rpcport       (default=\"\")             RPC port"
    echo "      --rpcaddress    (default=\"127.0.0.1\")    address where Prettywomancoin node runs (only needed when not running for local machine)"
    echo "      --prettywomancoincli    (default=\"prettywomancoin-cli\")  name of the prettywomancoin-cli executable set in the PATH or absolute path to the prettywomancoin-cli executable"
    ...
    echo "      --help                                   Get help and examples for prettywomancoin-diag.sh"
    echo ""
    echo "Examples: "   
    echo "      Running test on local machine and prettywomancoin-cli is in the directory: my_node/prettywomancoin_binaries"
    echo "          prettywomancoin-diag.sh --rpcuser test --rpcpassword test --rpcport 8899 --prettywomancoincli my_node/prettywomancoin_binaries/prettywomancoin-cli --test"
    echo "      Running diag script on local machine and prettywomancoin-cli is in the PATH"
    echo "          prettywomancoin-diag.sh --rpcuser test --rpcpassword test --rpcport 8899 > report.json"
    echo "      Running diag script on remote machine and prettywomancoin-cli is in the PATH (on local machine)"
    echo "          prettywomancoin-diag.sh --rpcuser test --rpcpassword test --rpcport 8899 --rpcaddress 123.123.123.123 > report.json"
    echo ""
    exit 0
fi
...
if [[ "$RPC_ADDRESS" == "127.0.0.1" || "$RPC_ADDRESS" == "localhost" ]]; then
    ...
    echo -n '"node_processes": '; do_cmd '"ERROR"' get_grep_prettywomancoind; echo ''
else
    echo -n '"Date": "'; do_cmd 'ERROR' get_date; echo '",'
