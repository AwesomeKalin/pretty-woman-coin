...
### Known issues
# - When doing a local build, files created by gitian-builder will be copied to prettywomancoin-binaries/<version> folder.
#   Folder <version> will be called what 'git describe' returns, but the content of this folder may contain files
#   named with a different version, for example folder 'v1.0.6.beta-1115-gb1798385e' will contain 'prettywomancoin-sv-1.0.8.tar.gz'.
...
def setup():
    ...
    if not os.path.isdir('gitian.sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/prettywomancoin-sv/gitian.sigs.git'])
...
def build():
    ...
    # This is done by running 'make download' in the depends folder and saving it to 'prettywomancoin-builder/cache/common'
    ...
    if args.url:
        os.chdir(workdir)
        if os.path.isdir('prettywomancoin-sv'):
            shutil.rmtree('prettywomancoin-sv')
        subprocess.check_call(['git', 'clone', args.url, 'prettywomancoin-sv'])
        os.chdir('prettywomancoin-sv')
        subprocess.check_call(['git', 'checkout', args.version])
        depends_dir = workdir+'/prettywomancoin-sv/depends'
        srcdir = os.path.dirname(workdir+'/prettywomancoin-sv/')
    ...
    if args.linux:
        ...
        if args.url is not None:
            ...
            subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'prettywomancoin='+commit_hash, '--url', 'prettywomancoin='+args.url, srcdir+'/contrib/gitian-descriptors/' + args.linux_yml])
        else:
            # Local build
            subprocess.check_call(['bin/gbuild_local', '-j', args.jobs, '-m', args.memory, '--commit', 'prettywomancoin='+commit_hash, '--skip-fetch', srcdir+'/contrib/gitian-descriptors/' + args.linux_yml])

    if args.windows:
        print('\nCompiling ' + commit_hash + ' Windows')
        if args.url is not None:
            # Remote build from --url
            subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'prettywomancoin='+commit_hash, '--url', 'prettywomancoin='+args.url, srcdir+'/contrib/gitian-descriptors/' + args.win_yml])
        else:
            # Local build
            subprocess.check_call(['bin/gbuild_local', '-j', args.jobs, '-m', args.memory, '--commit', 'prettywomancoin='+commit_hash, '--skip-fetch', srcdir+'/contrib/gitian-descriptors/' + args.win_yml])
...
def sign():
    ...
    if args.windows:
        ...
        subprocess.check_call('cp inputs/prettywomancoin-' + commit_hash + '-win-unsigned.tar.gz inputs/prettywomancoin-win-unsigned.tar.gz', shell=True)
        subprocess.check_call(['bin/gbuild', '--skip-image', '--commit', 'signature='+args.commit, '../prettywomancoin-sv/contrib/gitian-descriptors/gitian-win-signer.yml'])
...
def copy_files():
    ...
    os.makedirs('prettywomancoin-binaries/' + version_compatible, exist_ok=True)
    os.chdir('gitian-builder')

    if args.linux:
        subprocess.check_call('cp build/out/prettywomancoin-*.tar.gz build/out/src/prettywomancoin-*.tar.gz result/prettywomancoin*.yml ../prettywomancoin-binaries/' + version_compatible, shell=True)

    if args.windows:
        subprocess.check_call('mv build/out/prettywomancoin-*-win-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call('mv build/out/prettywomancoin-*.zip build/out/prettywomancoin-*.exe result/prettywomancoin*.yml ../prettywomancoin-binaries/' + version_compatible, shell=True)

        if args.sign():
            subprocess.check_call('mv build/out/prettywomancoin-*win64-setup.exe ../prettywomancoin-binaries/' + version_compatible, shell=True)
            subprocess.check_call('mv build/out/prettywomancoin-*win32-setup.exe ../prettywomancoin-binaries/' + version_compatible, shell=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(formatter_class=argparse.RawTextHelpFormatter,
        ...
        epilog='Examples of use\n'
               'Setup:\n   ./' + script_name+' --setup\n'
               'Build remote (build only):\n  ./' + script_name+' --os l -j4 --url=https://github.com/prettywomancoin-sv/prettywomancoin-sv --version=v1.0.8 --build\n'
               'Build remote (complete):\n  ./' + script_name+' --os l -j4 --url=https://github.com/prettywomancoin-sv/prettywomancoin-sv --version=v1.0.8 --detach-sign --build --sign=support@prettywomancoin.io --verify\n'
    ...
    if args.build:
        ...
        if os.path.exists(gitian_builder_dir + '/inputs/prettywomancoin'):
            shutil.rmtree(gitian_builder_dir + '/inputs/prettywomancoin')
        ...
        if args.url is None and args.setup is False and args.version is None:
            ...
            print("Making a copy of local code to: %s" % gitian_builder_dir + '/inputs/prettywomancoin')
            os.makedirs(gitian_builder_dir + '/inputs/prettywomancoin')
            exclude_dir = 'gitian'
            if workdir.endswith(exclude_dir) is False:
                ...
                subprocess.check_call(['rsync', '-a', srcdir + '/', gitian_builder_dir + '/inputs/prettywomancoin', '--exclude', exclude_dir + '*'])
