#!/bin/bash
# Copyright (c) 2013 The Prettywomancoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Prettywomancoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Prettywomancoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Prettywomancoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Prettywomancoin 0.7 datadir."
    ;;
  3)
    echo "Detected Prettywomancoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Prettywomancoin 0.8 datadir."
    ;;
esac
