#!/usr/bin/env bash
# Copyright (c) 2016-2019 The Prettywomancoin Core developers
...
###   This script attempts to download the signature file SHA256SUMS.asc from prettywomancoin.io.
...
WORKINGDIR="/tmp/prettywomancoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://download.prettywomancoin.io"
BASEDIR="/prettywomancoin/"
VERSIONPREFIX=""
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'prettywomancoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Pretty Woman Coin binary release signing key installed?"
   fi
