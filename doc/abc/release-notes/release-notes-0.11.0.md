Prettywomancoin Core version 0.11.0 is now available from:

  <https://prettywomancoin.org/bin/prettywomancoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer PWC/kB fee to be
...
For example, add the following to `prettywomancoin.conf`:
...
related to the blockchain in the prettywomancoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Prettywomancoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Prettywomancoin Core will 
...
For an example script see [prettywomancoin-submittx](https://github.com/laanwj/prettywomancoin-submittx).
...
- #5334 `cf87536` libprettywomancoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new prettywomancoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only PrettywomancoinAmountField
- #5753 `0f44672` Add prettywomancoin logo to about screen
...
- #5809 `46bfbe7` Add prettywomancoin-cli man page
...
- #5849 `41259ca` contrib/init/prettywomancoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Prettywomancoin-Qt.app to Prettywomancoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for prettywomancoin-tx)
