 ...
 */
class CPrettywomancoinSecret : public CBase58Data {
public:
    ...
    CPrettywomancoinSecret(const CKey &vchSecret) { SetKey(vchSecret); }
    CPrettywomancoinSecret() {}
};

template <typename K, int Size, CChainParams::Base58Type Type>
class CPrettywomancoinExtKeyBase : public CBase58Data {
public:
    ...
    CPrettywomancoinExtKeyBase(const K &key) { SetKey(key); }

    CPrettywomancoinExtKeyBase(const std::string &strBase58c) {
        SetString(strBase58c.c_str(), Params().Base58Prefix(Type).size());
    }

    CPrettywomancoinExtKeyBase() {}
};

typedef CPrettywomancoinExtKeyBase<CExtKey, BIP32_EXTKEY_SIZE,
                           CChainParams::EXT_SECRET_KEY>
    CPrettywomancoinExtKey;
typedef CPrettywomancoinExtKeyBase<CExtPubKey, BIP32_EXTKEY_SIZE,
                           CChainParams::EXT_PUBLIC_KEY>
    CPrettywomancoinExtPubKey;
