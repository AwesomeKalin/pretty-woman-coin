// Copyright (c) 2018 The Prettywomancoin Unlimited developers
...
// Not ideal - a modified copy of the options from prettywomancoin-cli 
                                            ...
                                            RandomIntGenerator & random_int_func)
{
    ...
    while ((GetTime() < start + searchDuration) && !found)
    {
        ...
        // and the block will be rejected.  So do not advance time (let it be advanced by prettywomancoind every time we
        // request a new block).
    ...
    while (0 != nblocks)
    {
        ...
        if (strPrint != "")
        {
            ...
            else
            {
                ...
                // The result is sent to prettywomancoind above when the loop gets to it.
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        std::string appname("prettywomancoin-miner");
        std::string usage = "\n" + _("Usage:") + "\n" + "  " + appname + " [options] " + "\n";
