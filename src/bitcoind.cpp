...
// Distributed under the Open PWC software license, see the accompanying LICENSE.
 ...
 *
 * This is the developer documentation of Pretty Woman Coin
 * (https://prettywomancoin.io/). Pretty Woman Coin is a client for the digital
 * currency called Pretty Woman Coin, which enables
 * instant payments to anyone, anywhere in the world. Pretty Woman Coin uses
...
bool AppInit(int argc, char *argv[]) {
    ...
    try {
        ...
        {
            fprintf(stderr, "Mandatory consensus parameter is not set. In order to start prettywomancoind you must set the "
                            "following consensus parameters: \"excessiveblocksize\" and "
                            "\"maxstackmemoryusageconsensus\". In order to start prettywomancoind with no limits you can set "
                            ...
                            "https://prettywomancoin.io/choosing-consensus-settings/");
            return false;
        }
        if (!gArgs.IsArgSet("-minminingtxfee"))
        {
            fprintf(stderr, "Mandatory policy parameter is not set. In order to start prettywomancoind you must set the "
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) &&
                !boost::algorithm::istarts_with(argv[i], "prettywomancoin:"))
                fCommandLine = true;

        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in "
                            "prettywomancoind anymore. Use the prettywomancoin-cli utility "
                            "instead.\n");
            exit(EXIT_FAILURE);
        }
        // -server defaults to true for prettywomancoind
...
#if HAVE_DECL_DAEMON
            fprintf(stdout, "Prettywomancoin server starting\n");
...
int main(int argc, char *argv[]) {
    ...
    // Connect prettywomancoind signal handlers
    noui_connect();
