...
class CPrettywomancoinLevelDBLogger : public leveldb::Logger {
public:
    ...
    void Logv(const char *format, va_list ap) override {
        ...
        for (int iter = 0; iter < 2; iter++) {
            ...
            if (p < limit) {
                ...
                // Do not use vsnprintf elsewhere in prettywomancoin source code, see
                // above.
...
static leveldb::Options GetOptions(size_t nCacheSize, size_t nMaxFiles) {
    ...
    options.info_log = new CPrettywomancoinLevelDBLogger();
