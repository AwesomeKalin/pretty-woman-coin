...
std::string HelpMessage(HelpMessageMode mode, const Config& config) {
    ...
    strUsage += HelpMessageOpt(
        "-printtoconsole",
        _("Send trace/debug info to console instead of prettywomancoind.log file"));
    strUsage += HelpMessageOpt("-shrinkdebugfile",
                               _("Shrink prettywomancoind.log file on client startup "
...
std::string LicenseInfo() {
    const std::string URL_SOURCE_CODE =
        "<https://github.com/prettywomancoin-sv/prettywomancoin-sv>";
    const std::string URL_WEBSITE = "<https://prettywomancoin.io>";
    ...
    return CopyrightHolders(
           ...
           strprintf(_("Distributed under the Open PWC software license, see the "
                       "accompanying file %s"),
                     "LICENSE") +
           "\n" + "\n" +
...
/** Sanity checks
 *  Ensure that Prettywomancoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Reopen prettywomancoind.log on SIGHUP
...
bool AppInitParameterInteraction(ConfigInit &config) {
    ...
    // Signal Prettywomancoin Cash support.
...
static bool LockDataDirectory(bool probeOnly) {
    ...
    // Make sure only a single Prettywomancoin process is using the data directory.
...
bool AppInitMain(ConfigInit &config, boost::thread_group &threadGroup,
    ...
    if (gArgs.GetBoolArg("-shrinkdebugfile", default_shrinkdebugfile)) {
        // Do this first since it both loads a bunch of prettywomancoind.log into memory,
        // and because this needs to happen before any other prettywomancoind.log printing.
