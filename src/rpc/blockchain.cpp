...
// Copyright (c) 2009-2019 The Prettywomancoin Core developers
...
UniValue getmempoolinfo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() != 0) {
        throw std::runtime_error(
            ...
            "  \"mempoolminfee\": xxxxx       (numeric) Minimum fee (in PWC/kB) for tx to be accepted\n"
            "}\n"
            "\nExamples:\n" +
...
Nothing (JSON null value) if successful and an error code otherwise.
    -1: Specified block cannot be marked as soft rejected.
        Response contains general error description while details are provided in prettywomancoind log file.
