...
static UniValue getinfo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() != 0) {
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total prettywomancoin "
...
static UniValue validateaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nReturn information about the given prettywomancoin address.\n"
            "\nArguments:\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Prettywomancoin address and we have full public key:
        if (pwallet) {
...
static UniValue createmultisig(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "are prettywomancoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) prettywomancoin address or hex-encoded public "
            "key\n"
            "       ,...\n"
            "     ]\n"
...
static UniValue echo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp) {
        throw std::runtime_error(
            ...
            "prettywomancoin-cli. There is no server-side difference.");
    }
...
static UniValue getsettings(const Config &config, const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
    {
        throw std::runtime_error(
            ...
            "  \"minminingtxfee\": xxxxx,                 (numeric) Lowest fee rate (in PWC/kB) for "
