                       ...
                       std::function<void()> httpCallback) 
{
    ...
    // Call into TxToJSON() in prettywomancoin-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in prettywomancoin-common, so we query them here and push the
    // data as JSON.
...
static UniValue createrawtransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            ...
            "key is the prettywomancoin address, the numeric value (can be string) is "
            "the " +
            CURRENCY_UNIT +
            " amount\n"
...
static UniValue getmerkleproof2(const Config& config, const JSONRPCRequest& request)
{
    ...
    // see also TSC description in  https://tsc.prettywomancoinassociation.net/standards/merkle-proof-standardised-format/?utm_source=Twitter&utm_medium=social&utm_campaign=Orlo
