...
#include "prettywomancoinconsensus.h"
...
inline int set_error(prettywomancoinconsensus_error *ret,
                     prettywomancoinconsensus_error serror) {
    if (ret) *ret = serror;
    return 0;
}
...
static bool verify_flags(unsigned int flags) {
    return (flags & ~(prettywomancoinconsensus_SCRIPT_FLAGS_VERIFY_ALL)) == 0;
}
...
static int verify_script(const CScriptConfig& config, const uint8_t* scriptPubKey,
                         ...
                         prettywomancoinconsensus_error* err) {
    if (!verify_flags(flags)) {
        return prettywomancoinconsensus_ERR_INVALID_FLAGS;
    }
    try {
        TxInputStream stream(SER_NETWORK, PROTOCOL_VERSION, txTo, txToLen);
        CTransaction tx(deserialize, stream);
        if (nIn >= tx.vin.size())
            return set_error(err, prettywomancoinconsensus_ERR_TX_INDEX);
        if (GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) != txToLen)
            return set_error(err, prettywomancoinconsensus_ERR_TX_SIZE_MISMATCH);

        // Regardless of the verification result, the tx did not error.
        set_error(err, prettywomancoinconsensus_ERR_OK);
        ...
        // Error deserializing
        return set_error(err, prettywomancoinconsensus_ERR_TX_DESERIALIZE);
    }
}

int prettywomancoinconsensus_verify_script_with_amount(
...
int prettywomancoinconsensus_verify_script(const CScriptConfig& config,
                                   const uint8_t *scriptPubKey,
                                   unsigned int scriptPubKeyLen,
                                   const uint8_t *txTo, unsigned int txToLen,
                                   unsigned int nIn, unsigned int flags,
                                   prettywomancoinconsensus_error *err) {
    if (flags & prettywomancoinconsensus_SCRIPT_ENABLE_SIGHASH_FORKID ||
        flags & prettywomancoinconsensus_SCRIPT_FLAGS_VERIFY_WITNESS_DEPRECATED) {
        return set_error(err, prettywomancoinconsensus_ERR_AMOUNT_REQUIRED);
    }
...
unsigned int prettywomancoinconsensus_version() {
