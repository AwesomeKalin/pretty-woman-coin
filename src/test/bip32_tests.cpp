// Copyright (c) 2013-2015 The Prettywomancoin Core developers
...
void RunTest(const TestVector &test) {
    ...
    for (const TestDerivation &derive : test.vDerive) {
        ...
        // Test private key
        CPrettywomancoinExtKey b58key;
        b58key.SetKey(key);
        BOOST_CHECK(b58key.ToString() == derive.prv);

        CPrettywomancoinExtKey b58keyDecodeCheck(derive.prv);
        CExtKey checkKey = b58keyDecodeCheck.GetKey();
        // ensure a base58 decoded key also matches
        assert(checkKey == key);

        // Test public key
        CPrettywomancoinExtPubKey b58pubkey;
        b58pubkey.SetKey(pubkey);
        BOOST_CHECK(b58pubkey.ToString() == derive.pub);

        CPrettywomancoinExtPubKey b58PubkeyDecodeCheck(derive.pub);
