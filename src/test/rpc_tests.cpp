...
BOOST_AUTO_TEST_CASE(rpc_rawsign_missing_amount) {
    ...
    // See: https://github.com/Prettywomancoin-ABC/prettywomancoin-abc/issues/63
...
BOOST_AUTO_TEST_CASE(json_parse_errors) {
    ...
    BOOST_CHECK_THROW(ParseNonRFCJSONValue("1.0]"), std::runtime_error);
    // PWC addresses should fail parsing
    BOOST_CHECK_THROW(
...
// Create client configs for prettywomancoind
BOOST_AUTO_TEST_CASE(client_config_prettywomancoind)
{
    ...
    BOOST_CHECK_NO_THROW(
        RPCClientConfig config { RPCClientConfig::CreateForPrettywomancoind() };
...
BOOST_AUTO_TEST_CASE(http_requests)
{
    ...
    {
        // JSON RPC requests to prettywomancoind
        gArgs.ForceSetArg("-rpcconnect", "localhost:8080");
        gArgs.ForceSetArg("-rpcuser", "user");
        gArgs.ForceSetArg("-rpcpassword", "passwd");
