...
const char *const BITCOIN_CONF_FILENAME = "prettywomancoin.conf";
const char *const BITCOIN_PID_FILENAME = "prettywomancoind.pid";
...
std::vector<std::string> ArgsManager::GetNonSensitiveParameters()
{
    ...
    // when starting prettywomancoind (e.g. prettywomancoin -name) or they will not be added to mapMultiArgs.
...
#else
    const char *pszModule = "prettywomancoin";
#endif
    if (pex)
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Prettywomancoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Prettywomancoin
// Mac: ~/Library/Application Support/Prettywomancoin
// Unix: ~/.prettywomancoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Prettywomancoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // Mac
    return pathRet / "Library/Application Support/Prettywomancoin";
#else
    // Unix
    return pathRet / ".prettywomancoin";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string &confPath) {
    ...
    // No prettywomancoin.conf file is OK
