...
// Copyright (c) 2017-2018 The Prettywomancoin developers
...
#if defined(NDEBUG)
#error "Prettywomancoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Prettywomancoin Signed Message:\n";
    ...
    if (!state.IsMissingInputs()) {
        ...
        if (fTxProcessingCompleted) {
            ...
            if (!state.CorruptionPossible()) {
                ...
                // https://github.com/prettywomancoin/prettywomancoin/issues/8279
                // for details.
                handlers.mpTxnRecentRejects->insert(tx.GetId());
            }
...
static void HandleInvalidStateForP2PNonOrphanTxn(
    ...
    if (TxValidationPriority::low == txStatus.mTxInputData->GetTxValidationPriority() ||
        ...
        if (!state.CorruptionPossible()) {
            ...
            // malleated. See https://github.com/prettywomancoin/prettywomancoin/issues/8279
            // for details.
            handlers.mpTxnRecentRejects->insert(tx.GetId());
            ...
            if (pindexOldTip) {
                ...
                if (!datarefs.empty()) {
                    ...
                    if (toRemove.size() != funds.size()) {
                        // if we mined them by error (calling prettywomancoin-cli generate for e.g.), then we have to
