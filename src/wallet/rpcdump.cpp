...
UniValue importprivkey(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            "importprivkey \"prettywomancoinprivkey\" ( \"label\" ) ( rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your "
            "wallet.\n"
            "\nArguments:\n"
            "1. \"prettywomancoinprivkey\"   (string, required) The private key (see "
            "dumpprivkey)\n"
    ...
    CPrettywomancoinSecret vchSecret;
...
UniValue importaddress(const Config &config, const JSONRPCRequest &request) {
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                           "Invalid Prettywomancoin address or script");
    }
...
UniValue dumpprivkey(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The prettywomancoin address for the "
            "private key\n"
            "\nResult:\n"
    ...
    return CPrettywomancoinSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "(either absolute or relative to prettywomancoind)\n"
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/prettywomancoin/prettywomancoin/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by Prettywomancoin %s\n", CLIENT_BUILD);
    ...
    if (!masterKeyID.IsNull()) {
        ...
        if (pwallet->GetKey(masterKeyID, key)) {
            ...
            CPrettywomancoinExtKey b58extkey;
    ...
    for (std::vector<std::pair<int64_t, CKeyID>>::const_iterator it =
        ...
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CPrettywomancoinSecret(key).ToString(),
                              strTime);
