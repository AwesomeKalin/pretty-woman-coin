...
class TestPrettywomancoinCli(PrettywomancoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info(
            "Compare responses from gewalletinfo RPC and `prettywomancoin-cli getwalletinfo`")
        cli_response = self.nodes[0].cli.getwalletinfo()
        rpc_response = self.nodes[0].getwalletinfo()
        assert_equal(cli_response, rpc_response)

        self.log.info(
            "Compare responses from getblockchaininfo RPC and `prettywomancoin-cli getblockchaininfo`")
