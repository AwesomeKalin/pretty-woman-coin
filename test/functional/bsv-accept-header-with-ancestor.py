...
# Scenario:
# 1. Create block B0.
# 2. Send HEADERS message of B0 to prettywomancoind. Prettywomancoind should send GETDATA for this block.
# 3. Send BLOCK message of B0 to prettywomancoind.
# 4. Prettywomancoind accepts block B0.
# 5. Create blocks B1 and B2.
# 6. Send HEADERS of B2 to prettywomancoind.
#    Prettywomancoind should not send GETDATA because previous block is missing.
# 7. Send HEADERS of B1 to prettywomancoind. It should be accepted (GETDATA should be received).
# 8. Send HEADERS of B2 to prettywomancoind. It should be accepted now that B1 is known (GETDATA should be received).
...
class AcceptHeaderWithAncestor(PrettywomancoinTestFramework):
    ...
    def run_test(self):
        ...
        with self.run_node_with_connections("reject headers if previous block is missing", 0, [], self.num_peers) as p2p_connections:
            ...
            # 2. Connection sends HEADERS msg to prettywomancoind and waits for GETDATA.
            ...
            # 3. Connection sends BLOCK to prettywomancoind.
            connection.cb.send_message(msg_block(block_0))

            # 4. Prettywomancoind adds block to active chain.
            ...
            # 6. Connection sends HEADERS of the second block to prettywomancoind. It should be rejected.
            headers_message = msg_headers()
            ...
            # 7. Connection sends HEADERS of the first block to prettywomancoind. It should be accepted.
            headers_message = msg_headers()
            ...
            # 8. Connection sends HEADERS of the second block to prettywomancoind. It should be accepted now that previous block is known.
            headers_message = msg_headers()
            headers_message.headers = [CBlockHeader(block_2)]
            connection.cb.send_message(headers_message)
