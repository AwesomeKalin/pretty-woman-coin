...
# Create 2 connections (connection1 and connection2) to prettywomancoind node and measure how long it takes for connection2 to receive a transaction that connection1 sends to prettywomancoind.
# 1. -broadcastdelay is set to 0 to calculate the minimal delay that network and prettywomancoind need for this functionality. This should not be too large.
...
class BroadcastDelayTest(PrettywomancoinTestFramework):
    ...
    def syncNodesWithTransaction(self, num_transactions, txs, connection1, connection2):
        ...
        for i in range(num_transactions):
            ...
            # node1 sends transaction to prettywomancoind
    ...
    def run_test(self):
        ...
        def run_pinging_connection(connection):
            # Connection3 is used here only for constantly pinging prettywomancoind node.
            # It is needed so that prettywomancoind is awake all the time (without 100ms sleeps).
