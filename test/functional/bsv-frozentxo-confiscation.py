...
from test_framework.test_framework import PrettywomancoinTestFramework, ChainManager
...
class FrozenTXOConfiscation(PrettywomancoinTestFramework):
    ...
    def check_log(self, node, line_text):
        for line in open(glob.glob(node.datadir + "/regtest/prettywomancoind.log")[0]):
            if re.search(line_text, line) is not None:
                self.log.debug("Found line in prettywomancoind.log: %s", line.strip())
                return True
        return False
    ...
    def _check_mempool_removal(self, node):
        ...
        assert not self.check_log(node.rpc, mempool_scan_check_log_string) # prettywomancoind should not unnecessarily scan whole mempool to find invalid confiscation transactions.
        ...
        assert_equal(node.rpc.getrawmempool(), [])
        assert self.check_log(node.rpc, mempool_scan_check_log_string) # prettywomancoind now should scan whole mempool
