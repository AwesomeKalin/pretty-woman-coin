...
#    2. Prettywomancoind sends GETDATA to peer1.
#    3. Peer1 sends block b1.
#    4. Peer2 creates new block b2 with height 2. Right after that, it sends headers message.
#    5. Prettywomancoind sends GETDATA to peer2.
#    6. Peer2 sends block b2.
#    7. Prettywomancoind logs that more honest block with the same height was received.

class LogTimeDiffTest(PrettywomancoinTestFramework):
    ...
    def run_test(self):
        ...
        with self.run_node_with_connections("logging difference between block created timestamp and header received timestamp", 0, [], 2) as p2p_connections:
            ...
            # 3. connection1 sends HEADERS msg to prettywomancoind and waits for GETDATA (received time is set)
            ...
            # 6. connection2 sends HEADERS msg to prettywomancoind
            headers_message = msg_headers()
            headers_message.headers = [CBlockHeader(block)]
            ...
            for line in open(glob.glob(self.options.tmpdir + "/node0" + "/regtest/prettywomancoind.log")[0]):
