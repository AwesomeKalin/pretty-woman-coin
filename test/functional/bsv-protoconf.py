#!/usr/bin/env python3
# Copyright (c) 2020 The Pretty Woman Coin developers
...
class BsvProtoconfTest(PrettywomancoinTestFramework):
    ...
    def start_node_with_protoconf(self, maxprotocolrecvpayloadlength, recvinvqueuefactor=0):
        ...
        Max size of prettywomancoin nodes' inv message is determined by maxprotocolrecvpayloadlength.
        ...
        # Send protoconf message from python node to prettywomancoind node
        ...
        # Receive prettywomancoind's protoconf and save max_recv_payload_length.
    ...
    def run_maxprotocolrecvpayloadlength_test(self, n_of_inv_to_send, maxprotocolrecvpayloadlength=0):
        ...
        # Check if maxprotocolrecvpayloadlength is not set and use default value from prettywomancoind
        if not maxprotocolrecvpayloadlength:
            maxprotocolrecvpayloadlength = 2 * LEGACY_MAX_PROTOCOL_PAYLOAD_LENGTH

        # Assert received max payload length from prettywomancoind node to be the same as we set in kwargs
        assert_equal(test_node.max_recv_payload_length, maxprotocolrecvpayloadlength)
        # Calculate maximum number of elements that prettywomancoind node is willing to receive
        maxInvElements = CInv.estimateMaxInvElements(test_node.max_recv_payload_length)
        logger.info(
            "Received prettywomancoind max message size: {} B, which represents {} elements. ".format(test_node.max_recv_payload_length, maxInvElements))
        ...
        logger.info("Received GetData from prettywomancoind.")
    ...
    def run_ban_test(self, maxprotocolrecvpayloadlength=0):
        ...
        maxInvElements = CInv.estimateMaxInvElements(test_node.max_recv_payload_length)
        logger.info(
            "Received prettywomancoind max message size: {} B, which represents {} elements. ".format(test_node.max_recv_payload_length,
                                                                                              maxInvElements))
        ### TEST WITH maxInvElements - 1, maxInvElements and maxInvElements + 1
        # 1. Send prettywomancoind Inv message that is smaller than max_recv_payload_length.
        ...
        # 2. Send prettywomancoind Inv message that is equal to max_recv_payload_length.
        ...
        # 3. Send prettywomancoind Inv message that is larger than max_recv_payload_length.
        logger.info( "Sending inv message with: {} elements. Max allowed : {}".format(maxInvElements + 1, maxInvElements))
    ...
    def run_recvinvqueuefactor_test(self, maxprotocolrecvpayloadlength, recvinvqueuefactor):
        ...
        # Send enough inv messages to fill the queue in prettywomancoind
    ...
    def run_test(self):
        ...
        # Send many INV messages and check when they fill up queue in prettywomancoind and some of them get missing
