...
def count_in_log(rpc, msg, node_dir, from_line=0):
    count = 0
    txes = set()
    for line in open(glob.glob(rpc.options.tmpdir + node_dir + "/regtest/prettywomancoind.log")[0]).readlines()[from_line:]:
        if msg in line:
...
class SendrawtransactionsSkipFlags(PrettywomancoinTestFramework):
    ...
    def run_test(self):
        ...
        logfile = open(glob.glob(self.options.tmpdir + "/node0/regtest/prettywomancoind.log")[0])
