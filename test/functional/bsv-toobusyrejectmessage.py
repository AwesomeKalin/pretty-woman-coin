...
# Scenario 1:
#   2 nodes (A and B) send HEADERS message to prettywomancoind. Prettywomancoind sends GetData to node A.
#   Node A then sends REJECT_TOOBUSY message. After that, node B should be asked for the same block (GetData).
# Scenario 2:
#   Node A sends HEADERS message to prettywomancoind. Prettywomancoind sends GetData to node A.
#   Node A sends REJECT_TOOBUSY message. Prettywomancoind waits and asks again after 5 seconds.

class TooBusyRejectMsgTest(PrettywomancoinTestFramework):
    ...
    def run_test(self):
        ...
        with self.run_node_with_connections("Scenario 2: sending TOOBUSY reject message with 1 node", 0, [], self.num_peers) as connections:
            ...
            # Prettywomancoind asks again after 5 seconds.
            wait_until(lambda: self.getDataLambda(connection.cb, block.sha256), lock=mininode_lock)
