...
class ChainstateWriteCrashTest(PrettywomancoinTestFramework):
    ...
    def restart_node(self, node_index, expected_tip):
        ...
        while time.time() - time_start < 120:
            try:
                ...
                # If prettywomancoind exits, then try again.  wait_for_node_exit()
                # should raise an exception if prettywomancoind doesn't exit.
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

        # If we got here, prettywomancoind isn't coming back up on restart. Could be a
        # bug in prettywomancoind, or -dbcrashnotbefore is not long enough?
