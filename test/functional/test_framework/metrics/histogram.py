"""Plot histograms and percentiles from the prettywomancoind.log
...
Find all prettywomancoind.log files in the specified locations, extract last logged
...
def find_last_experiment(base: Path) -> List[str]:
    last = base / sorted(base.rglob("prettywomancoind.log"))[-1].relative_to(base).parts[0]
    print("last:", repr(last))
    return list(map(str, last.rglob("prettywomancoind.log") if last.is_dir() else [last]))


def filter_experiments(dirs: List[str]) -> List[Path]:
    return [path
            for glob in (dir.rglob("prettywomancoind.log") if dir.is_dir() else [dir]
