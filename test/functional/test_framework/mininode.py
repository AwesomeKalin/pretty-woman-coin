...
# Copyright (c) 2010-2016 The Prettywomancoin Core developers
...
"""Prettywomancoin P2P network half-a-node.

This python code was modified from ArtForz' public domain  half-a-node, as
found in the mini-node branch of http://github.com/jgarzik/pynode.

NodeConn: an object which manages p2p connectivity to a prettywomancoin node
NodeConnCB: a base class that describes the interface for receiving
            callbacks with network messages from a NodeConn
CBlock, CTransaction, CBlockHeader, CTxIn, CTxOut, etc....:
    data structures that should map to corresponding structures in
    prettywomancoin/primitives
...
# Objects that map to prettywomancoind objects, which can be serialized/deserialized
...
class msg_headers():
    ...
    def deserialize(self, f):
        # comment in prettywomancoind indicates these should be deserialized as blocks
...
class NodeConnCB():
    """Callback and helper functions for P2P connection to a prettywomancoind node.
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True,
        ...
        with network_thread_loop_intent_lock, network_thread_loop_lock:
            ...
            logger.info('Connecting to Prettywomancoin Node: %s:%d' %
