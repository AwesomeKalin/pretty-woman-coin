...
class PrettywomancoinTestFramework():
    """Base class for a prettywomancoin test script.

    Individual prettywomancoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def __init__(self):
        ...
        self.prettywomancoind_proc_wait_timeout = BITCOIND_PROC_WAIT_TIMEOUT
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave prettywomancoinds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop prettywomancoinds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing prettywomancoind/prettywomancoin-cli (default: %default)")
        ...
        parser.add_option("--waitforpid", dest="waitforpid", default=False, action="store_true",
                          help="Display the prettywomancoind pid and wait for the user before proceeding. Useful for (eg) attaching a debugger to prettywomancoind.")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info(
                "Note: prettywomancoinds were not stopped and may still be running")
        ...
        if not self.options.nocleanup and not self.options.noshutdown and success != TestStatus.FAILED:
            ...
            if os.getenv("PYTHON_DEBUG", ""):
                ...
                filenames += glob.glob(os.path.join(self.options.tmpdir,
                                       "node*", "regtest", "prettywomancoind.log"))
    ...
    def start_nodes(self, extra_args=None):
        """Start multiple prettywomancoinds"""
        ...
        try:
            ...
            for i, node in enumerate(self.nodes):
                ...
                if(self.options.waitforpid):
                    ...
                    print('Do what you need (eg; gdb ./prettywomancoind {}) and then press <return> to continue...'.format(node.process.pid))
                    input()
        except:
    ...
    # This method runs and stops prettywomancoind node with index 'node_index'.
    # It also creates (and handles closing of) 'number_of_connections' connections to prettywomancoind node with index 'node_index'.
    ...
    # to prettywomancoind node with index 'node_index'.
    @contextlib.contextmanager
    ...
    def stop_node(self, i):
        """Stop a prettywomancoind test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped(timeout=self.prettywomancoind_proc_wait_timeout)

    def stop_nodes(self):
        """Stop multiple prettywomancoind test nodes"""
        for node in self.nodes:
        ...
        for node in self.nodes:
            ...
            node.wait_until_stopped(timeout=self.prettywomancoind_proc_wait_timeout)
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'prettywomancoind exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "prettywomancoind should have exited with an error"
                else:
                    assert_msg = "prettywomancoind should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as prettywomancoind's prettywomancoind.log with microprecision (so log files can be concatenated and sorted)
        ...
        if self.options.trace_rpc:
            rpc_logger = logging.getLogger("PrettywomancoinRPC")
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            # Create cache directories, run prettywomancoinds:
            for i in range(MAX_NODES):
                datadir = initialize_datadir(self.options.cachedir, i)
                args = [os.getenv("BITCOIND", "prettywomancoind"), "-server",
            ...
            for i in range(MAX_NODES):
                os.remove(log_filename(self.options.cachedir, i, "prettywomancoind.log"))
        ...
        for i in range(self.num_nodes):
            ...
            # Overwrite port/rpcport in prettywomancoin.conf
            initialize_datadir(self.options.tmpdir, i)
...
class ComparisonTestFramework(PrettywomancoinTestFramework):
    """Test framework for doing p2p comparison testing

    Sets up some prettywomancoind binaries:
    ...
    def __init__(self, destAddress = '127.0.0.1'):
        ...
        if not hasattr(self, "testbinary"):
            self.testbinary = [os.getenv("BITCOIND", "prettywomancoind")]
        if not hasattr(self, "refbinary"):
            self.refbinary = [os.getenv("BITCOIND", "prettywomancoind")]
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary", help="prettywomancoind binary to test")
        parser.add_option("--refbinary", dest="refbinary", help="prettywomancoind binary to use for reference nodes (if any)")
