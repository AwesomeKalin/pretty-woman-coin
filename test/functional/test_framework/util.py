...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError(
            "Fee of %s PWC too low! (Should be %s PWC)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError(
            "Fee of %s PWC too high! (Should be %s PWC)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting PWC values"""
...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "prettywomancoin.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
...
def get_auth_cookie(datadir):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "prettywomancoin.conf")):
        with open(os.path.join(datadir, "prettywomancoin.conf"), 'r', encoding='utf8') as f:
            for line in f:
...
def check_for_log_msg(rpc, log_msg, node_dir=None):
    """
    Checks for occurrence of the log_msg in the prettywomancoind.log
    ...
    if node_dir is None:
        ...
        logfile_path = os.path.join(rpc.datadir, "regtest", "prettywomancoind.log")
    else:
        logfile_path = glob.glob(rpc.options.tmpdir + node_dir + "/regtest/prettywomancoind.log")[0]
...
def count_log_msg(rpc, log_msg, node_dir=None):
    """
    Checks for number of occurrences of the log_msg in the prettywomancoind.log
    rpc can be any object which has .log member (logger)
    If node_dir is None, the rpc must be an TestNode instance and the logfile to search will be the one associated with this TestNode instance.
    """
    assert hasattr(rpc, "log")
...
def check_zmq_test_requirements(configfile, skip_test_exception):
    # Check that prettywomancoin has been built with ZMQ enabled
    ...
    # if we built prettywomancoind with ZMQ enabled, then we need zmq package to test its functionality
    try:
        import zmq
    except ImportError:
