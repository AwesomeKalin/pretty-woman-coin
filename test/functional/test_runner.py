...
`test/functional/test_framework/test_framework.py:PrettywomancoinTestFramework.main`.
...
def main():
    ...
    parser.add_argument('--watch', type=str,
                        default=None, help="Showing specified file in the console and monitoring its changes, usefull "
                                           ...
                                           "it will show prettywomancoind.log file of the specified node")
    ...
    tmpdir = os.path.join("%s", "prettywomancoin_test_runner_%s") % (
    ...
    enable_prettywomancoind = config["components"].getboolean("ENABLE_BITCOIND")

    if not (enable_wallet and enable_utils and enable_prettywomancoind):
        print(
            "No functional tests to run. Wallet, utils, and prettywomancoind must all be enabled")
        print(
...
def run_tests(test_list, build_dir, tests_dir, junitouput, exeext, tmpdir, jobs=1, enable_coverage=False, args=[],  build_timings=None, buildconfig="", file_for_monitoring=None, console=False, solo_position_start=-1):
    # Warn if prettywomancoind is already running (unix only)
    try:
        pidofOutput = subprocess.check_output(["pidof", "prettywomancoind"])
        if pidofOutput is not None and pidofOutput != b'':
            print("%sWARNING!%s There is already a prettywomancoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (
                BOLD[1], BOLD[0]))
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = os.path.join(
            build_dir, 'src', buildconfig, 'prettywomancoind' + exeext)
        os.environ["BITCOINCLI"] = os.path.join(
            build_dir, 'src', buildconfig, 'prettywomancoin-cli' + exeext)

    if not os.path.isfile(os.environ["BITCOIND"]):
        print("%sERROR!%s Can not find prettywomancoind executable here: %s. " % (
            BOLD[1], BOLD[0], os.environ["BITCOIND"]))
        sys.exit(0)

    if not os.path.isfile(os.environ["BITCOINCLI"]):
        print("%sERROR!%s Can not find prettywomancoin-cli executable here: %s. " % (
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None, file_for_monitoring=None, console=False, solo_position_start=-1):
        ...
        # In case there is a graveyard of zombie prettywomancoinds, we can apply a
        ...
        if (file_for_monitoring is not None):
            if file_for_monitoring[:4] == "node" and file_for_monitoring[4:].isnumeric():
                file_for_monitoring = os.path.join(file_for_monitoring, "regtest", "prettywomancoind.log")
...
class RPCCoverage():
    ...
    commands per `prettywomancoin-cli help` (`rpc_interface.txt`).
